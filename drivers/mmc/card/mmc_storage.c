...
#ifdef MMC_SECURE_STORAGE_REPAIR
			memset(data_buf, 0x0, sizeof(struct mmc_storage_head_t));
			tmp_node.offset_addr = MMC_STORAGE_OFFSET;
			ret = mmc_storage_rw_kernel(device, &tmp_node, (unsigned char *)data_buf, MMC_STORAGE_AREA_SIZE, 0);
			if (!ret) {
				ret = storage_check(device,data_buf);
				if (!ret) {
					data_buf->timestamp -= 1;
					tmp_node.offset_addr = MMC_STORAGE_OFFSET + MMC_STORAGE_AREA_SIZE;
					ret = mmc_storage_rw_kernel(device, &tmp_node, (unsigned char *)data_buf, MMC_STORAGE_AREA_SIZE, 1);
					if (!ret) {
						repair_flag = 1;
					} else
