...
/*#define STK_ROCKCHIP_PLATFORM*/
	...
	#include "linux/sensor/stk8baxx.h"
#endif

#define STK_ACC_DRIVER_VERSION	"3.5.2"

/*-----------User-defined settings---------------*/
#define STK_DEBUG_PRINT
/*#define STK_DEBUG_PRINT_DATA*/
...
#else
	#define ADDITIONAL_GPIO_CFG 0
#endif

#define STK8BAXX_I2C_NAME	"stk8baxx"
#define CHECK_CODE_SIZE          22

/*0xC:150Hz, 0xB:75Hz, 0xA:37.5Hz */
#define STK8BAXX_INIT_ODR		0xB
#define STK8BAXX_SPTIME_NO		4
const static int STK8BAXX_SAMPLE_TIME[STK8BAXX_SPTIME_NO] = {
	53333, 26667, 13333, 6667
};
...
#elif defined(IS_STK8BA22)
	#define STK_TUNE_XYOFFSET	20
	#define STK_TUNE_ZOFFSET	40
	#define STK_TUNE_NOISE		6
#endif
...
static int event_since_en_limit;
...
static int stk_i2c_rx(char *rxData, int length)
{
	...
	for (retry = 0; retry <= 3; retry++) {
		if (i2c_transfer(stk8baxx_data_ptr->client->adapter, msgs, 2)
			> 0)
			break;
		else
			mdelay(10);
	...
	if (retry > 3) {
		pr_err("stk8baxx i2c_transfer error, retry over 3\n");
		return -EIO;
	} else
		return 0;
}
...
static int stk_i2c_tx(char *txData, int length)
{
	...
	for (overall_retry = 0; overall_retry < 3; overall_retry++) {
		for (tx_retry = 0; tx_retry <= 3; tx_retry++) {
			if (i2c_transfer(stk8baxx_data_ptr->client->adapter,
				msg, 1) > 0)
				break;
			else
				mdelay(10);
		...
		if (tx_retry > 3) {
			pr_err("i2c_transfer error, tx_retry over 3\n");
			return -EIO;
		}
		...
		if (result < 0) {
			pr_err("stk8baxx i2c_transfer fail to read back\n");
			return result;
		}

		return 0;
	}
	pr_err("stk8baxx i2c_transfer read back error,w=0x%x,r=0x%x\n",
		txData[1], buffer);
	return -EIO;
}
...
static int gsensor_fetch_sysconfig_para(void)
{
	...
	ret = script_parser_fetch("gsensor_para", "gsensor_used",
		&device_used, 1);
	...
	if (1 == device_used) {
		if (SCRIPT_PARSER_OK != script_parser_fetch_ex("gsensor_para",
			"gsensor_name", (int *)(&name),
...
static void STK8BAXX_CheckInit(void)
{
	...
	for (i = 0; i < CHECK_CODE_SIZE; i++)
		for (j = 0; j < CHECK_CODE_SIZE; j++)
			stkcheckcode[i][j] = (char)STK8BAXX_GetCode(i, j);
}
...
static int STK8BAXX_Init(struct stk8baxx_data *stk, struct i2c_client *client)
{
	...
	pr_info("%s:issue sw reset to 0x%x, result=%d\n",
	...
	/*	ODR = 75 Hz	*/
	result = stk8baxx_smbus_write_byte_data(STK8BAXX_BWSEL,
		STK8BAXX_INIT_ODR);
	if (result < 0) {
		pr_err("%s:failed to write reg 0x%x, error=%d\n",
	...
	result = stk8baxx_smbus_write_byte_data(STK8BAXX_POWMODE,
		STK8BAXX_MD_SUSPEND);
	if (result < 0) {
		pr_err("%s:failed to write reg 0x%x, error=%d\n",
			__func__, STK8BAXX_POWMODE, result);
	...
	pr_err("%s:return to suspend mode after init\n", __func__);
...
#ifdef STK_LOWPASS
static void STK8BAXX_LowPass(struct stk8baxx_data *stk,
	struct stk8baxx_acc *acc_lp)
{
	...
	if (atomic_read(&stk->fir_en)) {
		if (stk->fir.num < firlength) {
			...
			/*
			pr_info("add [%2d] [%5d %5d %5d] => [%5d %5d %5d] : [%5d %5d %5d]\n", idx,
	...
	struct stk8baxx_acc *acc_xyz)
{
	...
	if (event_since_en >= STK_TUNE_DELAY) {
		...
		if (stk_tune_index == STK_TUNE_NUM) {
			for (ii = 0; ii < 3; ii++) {
				if ((stk_tune_max[ii] - stk_tune_min[ii])
					> STK_TUNE_NOISE) {
					stk_tune_index = 0;
					STK8BAXX_ResetPara();
					return;
			...
			if (acc[2] > 0)
				stk_tune_offset[2] =
					stk_tune_sum[2]/STK_TUNE_NUM - LSB_1G;
			else
				stk_tune_offset[2] =
					stk_tune_sum[2]/STK_TUNE_NUM - (-LSB_1G);
			...
			event_since_en = 0;
			pr_info(KERN_INFO "%s:TUNE done, %d,%d,%d\n", __func__,
...
#ifdef IS_STK8BA50
static int STK8BAXX_CheckReading(struct stk8baxx_data *stk,
	s16 acc[], bool clear)
{
	static int check_result;

	if (acc[0] == 511 || acc[0] == -512 || acc[1] == 511
		|| acc[1] == -512 || acc[2] == 511 || acc[2] == -512) {
	...
	if (clear) {
		if (check_result >= 3) {
			if (acc[0] != 511 && acc[0] != -512
				&& acc[1] != 511 && acc[1] != -512) {
				event_since_en_limit = 21;
				if (atomic_read(&stk->firlength) < 2) {
					atomic_set(&stk->firlength, 2);
					memset(&stk->fir, 0x00, sizeof(stk->fir));
					atomic_set(&stk->fir_en, 1);
				}
			} else
				event_since_en_limit = 10000;
			pr_info("%s: incorrect reading\n", __func__);
			return 1;
		}
	}
	return 0;
...
static int STK8BAXX_ReadSensorData(struct stk8baxx_data *stk)
{
	...
	if (result < 0) {
		pr_err("%s: failed to read acc data, error=%d\n",
			__func__, result);
		return result;
	}
/*
...
*/
...
#ifdef STK_DEBUG_PRINT
	/*
	pr_info("%s: before trans, \t\t\t\t raw_acc=%4d, %4d, %4d\n",
	...
	*/
	...
	stk->acc_xyz.z = acc.z;
	mutex_unlock(&stk->value_lock);
#ifdef STK_DEBUG_PRINT_DATA
	pr_info("stk8baxx acc= %4d, %4d, %4d\n", (int)stk->acc_xyz.x,
		 (int)stk->acc_xyz.y, (int)stk->acc_xyz.z);
#endif
	return 0;
}
#elif (defined(IS_STK8BA22))
	...
	s16 acc[], bool clear)
{
	...
	if (clear) {
		if (check_result >= 3) {
			if (acc[0] != 127 && acc[0] != -128 && acc[1] != 127
				&& acc[1] != -128) {
				event_since_en_limit = 21;
				if (atomic_read(&stk->firlength) < 2) {
					atomic_set(&stk->firlength, 2);
...
#endif
	aml_sensor_report_acc(stk->client, stk->input_dev,
		stk->acc_xyz.x, stk->acc_xyz.y, stk->acc_xyz.z);
	mutex_unlock(&stk->value_lock);
	return 0;
}
...
#ifdef STK_DEBUG_PRINT
	pr_info("%s:sdelay_us=%u, sr_no=0x%x\n",
		__func__, sdelay_ns/1000, sr_no);
#endif

	result = STK8BAXX_GetEnable(stk, &en);
	if (result < 0)
		pr_err("%s: STK8BAXX_GetEnable failed, error=%d\n",
			__func__, result);
...
static s64 STK8BAXX_GetDelay(struct stk8baxx_data *stk)
{
	...
	if (sample_time < 0) {
		pr_err("%s: failed to read reg 0x%x, error=%d\n",
...
static int STK8BAXX_GetOffset(struct stk8baxx_data *stk, u8 offset[])
{
	...
	result = stk8axxx_smbus_read_i2c_block_data(STK8BAXX_OFSTFILTX,
		3, offset);
	if (result < 0) {
		pr_err("%s:failed to write reg 0x%x, error=%d\n",
			__func__, STK8BAXX_OFSTFILTX, result);
	...
	if (result < 0) {
		pr_err( "%s:failed to read from reg 0x%x, error=%d\n",
			__func__, STK8BAXX_OFSTFILTX, result);
		return result;
	}
	offset[0] = result;
...
static int STK8BAXX_SetRange(struct stk8baxx_data *stk, char srange)
{
	...
	if (srange > STK8BAXX_RANGE_MASK) {
		pr_err("%s:range=0x%x, out of range\n", __func__, srange);
		return -EINVAL;
	}
#ifdef STK_DEBUG_PRINT
	pr_info("%s:range=0x%x\n", __func__, srange);
#endif
	switch (srange) {
	...
	default:
		write_buffer = STK8BAXX_RNG_2G;
		pr_err("%s: unknown range, set as STK8BAXX_RNG_2G\n", __func__);
	}
	...
	if (en == 0)
		STK8BAXX_EnterActive(stk);
	result = stk8baxx_smbus_write_byte_data(STK8BAXX_RANGESEL,
		write_buffer);
	if (result < 0) {
		pr_err("%s:failed to write reg 0x%x, error=%d\n",
			__func__, STK8BAXX_RANGESEL, result);
...
static int STK8BAXX_GetCali(struct stk8baxx_data *stk)
{
...
#ifdef STK_TUNE
	pr_info("%s: stk_tune_done=%d, stk_tune_index=%d, stk_tune_offset=%d,%d,%d\n", __func__,
	...
	if ((stk8baxx_get_file_content(r_buf, STK_ACC_CALI_FILE_SIZE)) == 0) {
		if (r_buf[0] == STK_ACC_CALI_VER0 && r_buf[1] == STK_ACC_CALI_VER1) {
			...
			pr_info("%s: offset:%d,%d,%d, mode=0x%x\n", __func__,
							r_buf[2], r_buf[3], r_buf[4], r_buf[5]);
		} else
			pr_err("%s: cali version number error! r_buf=0x%x,0x%x,0x%x,0x%x,0x%x\n",
...
static int STK8BAXX_SetOfstTarget(struct stk8baxx_data *stk)
{
	...
	pr_info("%s: read az=%d, target=0x%x\n", __func__, raw_acc_z, target);
...
static int STK8BAXX_SetKEachAxis(struct stk8baxx_data *stk)
{
	...
	result = stk8baxx_smbus_write_byte_data(STK8BAXX_OFSTCOMP1,
		CAL_AXIS_Y_EN);
	if (result < 0) {
		pr_err("%s:failed to write reg 0x%x, error=%d\n",
			__func__, STK8BAXX_OFSTCOMP1, result);
...
static int STK8BAXX_SetCali(struct stk8baxx_data *stk, char sstate)
{
	...
	if (result < 0) {
		pr_err("%s:failed to STK8BAXX_SetDelay, error=%d\n",
			__func__, result);
	...
	if (result < 0) {
		pr_err("%s:failed to STK8BAXX_SetOfstTarget, error=%d\n",
			__func__, result);
		atomic_set(&stk->cali_status, STK_K_FAIL_I2C);
		goto k_exit;
	}
	...
	if (result < 0) {
		pr_err("%s:failed to STK8BAXX_SetKEachAxis, error=%d\n",
			__func__, result);
		atomic_set(&stk->cali_status, STK_K_FAIL_I2C);
		goto k_exit;
	}
	...
	pr_info("%s: after compensation, offset=0x%x,0x%x,0x%x\n",
		__func__, offset[0], offset[1], offset[2]);
	result = stk8baxx_store_in_file(offset, STK_K_SUCCESS_FILE);
	if (result < 0) {
		pr_err("%s:failed to stk8baxx_store_in_file, error=%d\n",
			__func__, result);
	...
	if (enabled) {
#if STK_ACC_POLLING_MODE
		hrtimer_start(&stk->acc_timer,
			stk->poll_delay, HRTIMER_MODE_REL);
#else
	...
	pr_info("%s: successful calibration\n", __func__);
	return 0;
...
static void STK8BAXX_LoadCali(struct stk8baxx_data *stk)
{
	...
	if ((stk8baxx_get_file_content(r_buf, STK_ACC_CALI_FILE_SIZE)) == 0) {
		if (r_buf[0] == STK_ACC_CALI_VER0
			...
			pr_info("%s: set offset:%d,%d,%d, mode=%d\n", __func__,
			...
			pr_err("%s: cali version number error! r_buf=0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n",
...
/*SYSCALL_DEFINE3(fchmodat, int, dfd,
...
static int32_t stk8baxx_get_file_content(char *r_buf, int8_t buf_size)
{
	...
	if (IS_ERR(cali_file)) {
		pr_err("%s: filp_open error, no offset file!\n", __func__);
		return -ENOENT;
	} else {
		fs = get_fs();
		set_fs(get_ds());
...
static int stk8baxx_store_in_file(u8 offset[], u8 status)
{
	...
	if (IS_ERR(cali_file)) {
		...
		for (i = 0; i < STK_ACC_CALI_FILE_SIZE; i++) {
			if (r_buf[i] != w_buf[i]) {
				pr_err("%s: read back error, r_buf[%x](0x%x) != w_buf[%x](0x%x)\n", __func__, i, r_buf[i], i, w_buf[i]);
				filp_close(cali_file, NULL);
		...
		const char *buf, size_t count)
{
	...
	if (error) {
		pr_err("%s: strict_strtoul failed, error=%d\n",
			__func__, error);
		return error;
	}
	...
	if ((data == 0) || (data == 1))
		...
		pr_err("%s: invalud argument, data=%ld\n", __func__, data);
	return count;
}
		...
		const char *buf, size_t count)
{
	...
	pr_info("Please use  ms to set delay!\n");
	STK8BAXX_SetDelay(stk, data*1000*1000);
	return count;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	u8 offset[3] = {0};
	STK8BAXX_GetOffset(stk, offset);
	pr_info("%s: offset = 0x%x, 0x%x, 0x%x\n",
		__func__, offset[0], offset[1], offset[2]);
		...
		const char *buf, size_t count)
{
	...
	error = strict_strtoul(token[0], 16, (unsigned long *)&(r_offset[0]));
	if (error < 0) {
		pr_err("%s:strict_strtoul failed, error=%d\n", __func__, error);
		return error;
	}
	error = strict_strtoul(token[1], 16, (unsigned long *)&(r_offset[1]));
	if (error < 0) {
		pr_err("%s:strict_strtoul failed, error=%d\n", __func__, error);
		return error;
	}
	error = strict_strtoul(token[2], 16, (unsigned long *)&(r_offset[2]));
	if (error  < 0) {
		pr_err("%s:strict_strtoul failed, error=%d\n", __func__, error);
		return error;
	}
		...
		const char *buf, size_t count)
{
	...
	for (i = 0; i < 2; i++)
		token[i] = strsep((char **)&buf, " ");
	error = strict_strtoul(token[0], 16, (unsigned long *)&(addr));
	if (error < 0) {
		pr_err("%s:strict_strtoul failed, error=%d\n", __func__, error);
		return error;
	}
	error = strict_strtoul(token[1], 16, (unsigned long *)&(cmd));
	if (error < 0) {
		pr_err("%s:strict_strtoul failed, error=%d\n", __func__, error);
		return error;
	}
	pr_info("%s: write reg 0x%x=0x%x\n", __func__, addr, cmd);
		...
		const char *buf, size_t count)
{
	...
	if (result) {
		pr_err("%s: strict_strtoul failed, result=%d\n",
			__func__, result);
		return result;
	}
	...
	if (result < 0) {
		pr_err("%s: failed to read reg 0x%x, result=%d\n",
...
#else
	pr_err("%s: firlen is not supported\n", __func__);
#endif
	return count;
}
...
static DEVICE_ATTR(enable, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
	stk8baxx_enable_show, stk8baxx_enable_store);
static DEVICE_ATTR(value, S_IRUGO, stk8baxx_value_show, NULL);
static DEVICE_ATTR(delay, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
	stk8baxx_delay_show, stk8baxx_delay_store);
static DEVICE_ATTR(cali, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
	stk8baxx_cali_show, stk8baxx_cali_store);
static DEVICE_ATTR(offset, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
	stk8baxx_offset_show, stk8baxx_offset_store);
static DEVICE_ATTR(send, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
	stk8baxx_send_show, stk8baxx_send_store);
static DEVICE_ATTR(recv, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
	stk8baxx_recv_show, stk8baxx_recv_store);
static DEVICE_ATTR(firlen, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
	stk8baxx_firlen_show, stk8baxx_firlen_store);
...
static long stk8baxx_ioctl(struct file *file,
	unsigned int cmd, unsigned long arg)
{
	...
	case STK_IOCTL_WRITE:
		...
		result = stk8baxx_smbus_write_byte_data(rwbuf[1], rwbuf[2]);
		if (result < 0) {
			pr_err("%s:failed to write reg 0x%x, error=%d\n",
	...
	case STK_IOCTL_READ:
		...
		result = stk8axxx_smbus_read_i2c_block_data(char3_buffer[1],
			char3_buffer[0], &rwbuf[1]);
		if (result < 0) {
			retval = result;
			goto ioctl_err;
		}
...
{
	struct stk8baxx_data *stk = container_of(work,
		struct stk8baxx_data, stk_work);
	STK8BAXX_ReadSensorData(stk);
...
{
	struct stk8baxx_data *stk = container_of(timer,
		struct stk8baxx_data, acc_timer);
	...
	struct stk8baxx_data *stk_dat, int int_pin)
{
	...
	error = request_any_context_irq(client->irq, stk_mems_irq_fun,
		IRQF_TRIGGER_RISING, "stk8baxx", stk_dat);
	if (error < 0) {
		pr_err("%s: request_threaded_irq(%d) failed for (%d)\n",
...
#if 0
#ifdef CONFIG_OF
static struct stk8baxx_platform_data *stk8baxx_parse_dt(struct device *dev)
{
	...
	if (!pdata) {
		dev_err(dev, "Could not allocate struct stk8baxx_platform_data");
		return NULL;
	}
	...
	ret = of_property_read_u32(np, "interrupt_pin", &pdata->interrupt_pin);
	if (ret) {
		dev_err(dev, "fail to get interrupt_pin\n");
		goto fail;
	}

	pdata->int_pin = of_get_gpio(np, 0);
	if (pdata->int_pin < 0) {
		dev_err(dev, "fail to get int_pin\n");
		goto fail;
	}
	...
	const struct i2c_device_id *id)
{
...
#endif
#ifdef IS_STK8BA22
	pr_info("stk8ba22_probe: driver version:%s\n", STK_ACC_DRIVER_VERSION);
#elif defined IS_STK8BA50
	pr_info("stk8ba50_probe: driver version:%s\n", STK_ACC_DRIVER_VERSION);
#endif

	if (!i2c_check_functionality(client->adapter,
		I2C_FUNC_SMBUS_BYTE_DATA | I2C_FUNC_SMBUS_I2C_BLOCK)) {
		error = i2c_get_functionality(client->adapter);
		pr_err("%s:i2c_check_functionality error, functionality=0x%x\n",
			__func__, error);
		error = -ENODEV;
	...
	if (np && !stk_platdata) {
		stk_platdata = stk8baxx_parse_dt(&client->dev);
		if (stk_platdata)
			client->dev.platform_data = stk_platdata;
		if (!stk_platdata) {
			pr_err("%s: Please set stk8baxx platform data!\n",
				__func__);
			err = -EINVAL;
	...
	stk->poll_delay = ns_to_ktime(STK8BAXX_SAMPLE_TIME[STK8BAXX_INIT_ODR-STK8BAXX_SPTIME_BASE]);
	...
	if (error) {
		pr_err("%s:stk8baxx initialization failed\n", __func__);
	...
	stk->input_dev->name = STK8BAXX_I2C_NAME;
